#!/usr/bin/env bash
# Chained subtask evaluation for small4 EfficiencyRouter on R+C questions (GPU).
# Runs eval.py with --qid-filter, producing full-task + overall + chained subtask.
# Overwrites rc_questions/small4/subtask_results.json with chained fields included.
#
# Prerequisites:
...
sbatch \
    --job-name=effrouter-rc-chain-s4 \
    --account=allcs \
    --partition=pvl \
    --gres=gpu:a6000:1 \
    --time=0:20:00 \
    --mem=16G \
    --cpus-per-task=2 \
    --output="$LOGS_DIR/eval_rc_small4_chained_%j.out" \
    --error="$LOGS_DIR/eval_rc_small4_chained_%j.out" \
    --wrap="cd $SCRIPT_DIR && \
...
echo "Submitted effrouter-rc-chain-s4. Logs: $LOGS_DIR/eval_rc_small4_chained_<jobid>.out"
