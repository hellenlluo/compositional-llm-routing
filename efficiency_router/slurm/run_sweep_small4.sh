#!/usr/bin/env bash
# Like run_sweep_no_qwen3.sh but restricted to the 4 small/weak models:
#   mistral-7b-instruct-v0.3, qwen1.5-0.5b-chat,
#   phi-4-mini-instruct, llama-3.1-nemotron-nano-8b
# Called by submit_sweep_small4.sh as the SLURM job script.
...
for OFFSET in "${OFFSETS[@]}"; do
    TAG="offset_${OFFSET}"
    CKPT="$SCRIPT_DIR/checkpoints/effrouter_small4_${TAG}.pt"
    ...
    echo ""
    echo "════════════════════════════════════════════════════"
    echo "  TRAIN  offset=$OFFSET  (small4)"
    echo "════════════════════════════════════════════════════"
    "$CONDA_PYTHON" train.py \
        --offset "$OFFSET" \
        --exclude-models $EXCLUDED \
        --ckpt-name "effrouter_small4_${TAG}"

    echo ""
    echo "════════════════════════════════════════════════════"
    echo "  EVAL   offset=$OFFSET  (small4)"
    echo "════════════════════════════════════════════════════"
