#!/usr/bin/env bash
# Train the BCE-loss perf router restricted to the 4 small/weak models (small4 ablation).
#   mistral-7b-instruct-v0.3, qwen1.5-0.5b-chat,
#   phi-4-mini-instruct, llama-3.1-nemotron-nano-8b
#
# Usage:
#   bash perf_router/training/submit_train_perf_router_bce_small4.sh
...
SAVE_PATH="/n/fs/scratch/dl3533/models/perf_router/perf_router_bce_small4.pt"

sbatch \
    --job-name="perf_router_bce_small4" \
    --account=pvl \
    --gres=gpu:a6000:1 \
    --time=00:45:00 \
    --mem=16G \
    --cpus-per-task=2 \
    --output="$SLURM_LOGS/train_perf_router_bce_small4_%j.out" \
    --error="$SLURM_LOGS/train_perf_router_bce_small4_%j.out" \
    --wrap="cd $REPO_ROOT && \
...
echo "Submitted perf_router_bce_small4. Logs: $SLURM_LOGS/train_perf_router_bce_small4_<jobid>.out"
