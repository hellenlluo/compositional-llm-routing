#!/usr/bin/env bash
# Evaluate the no-qwen3 perf router checkpoint.
# Results saved to perf_router/results/no_qwen3/

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
SLURM_LOGS="$REPO_ROOT/perf_router/logs"
RESULTS_DIR="$REPO_ROOT/perf_router/results/no_qwen3"
...
sbatch \
    ...
    --mem=16G \
    --cpus-per-task=2 \
    --output="$SLURM_LOGS/eval_perf_no_qwen3_%j.out" \
    --error="$SLURM_LOGS/eval_perf_no_qwen3_%j.out" \
    --wrap="cd $REPO_ROOT && \
...
echo "Submitted eval_perf_no_qwen3. Logs: $SLURM_LOGS/eval_perf_no_qwen3_<jobid>.out"
