#!/usr/bin/env bash
# Evaluate the BCE-loss no-qwen3 perf router checkpoint.
# Results saved to perf_router/results/bce_no_qwen3/
#
# Run this first; then run submit_eval_perf_router_rc_bce_no_qwen3.sh for R+C metrics.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
SLURM_LOGS="$REPO_ROOT/perf_router/logs"
RESULTS_DIR="$REPO_ROOT/perf_router/results/bce_no_qwen3"
...
sbatch \
    --job-name="eval_perf_bce_no_qwen3" \
    --account=pvl \
    --gres=gpu:a6000:1 \
    --time=0:30:00 \
    --mem=16G \
    --cpus-per-task=2 \
    --output="$SLURM_LOGS/eval_perf_bce_no_qwen3_%j.out" \
    --error="$SLURM_LOGS/eval_perf_bce_no_qwen3_%j.out" \
    --wrap="cd $REPO_ROOT && \
...
echo "Submitted eval_perf_bce_no_qwen3. Logs: $SLURM_LOGS/eval_perf_bce_no_qwen3_<jobid>.out"
