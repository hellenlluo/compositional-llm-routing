#!/usr/bin/env bash
# Chained subtask evaluation for BCE small4 perf router on R+C questions (GPU).
# Runs eval_perf_router.py with --qid-filter, producing full-task + overall + chained.
# Overwrites rc_questions/bce_small4/subtask_summary.json with chained fields included.
#
# Prerequisites:
#   1. python router_analysis/generate_rc_qids.py   (outputs/rc_qids.json)
#   2. bash perf_router/eval/submit_eval_perf_router_bce_small4.sh  (checkpoint must exist)
...
sbatch \
    ...
    --mem=16G \
    --cpus-per-task=2 \
    --output="$SLURM_LOGS/eval_perf_rc_bce_small4_chained_%j.out" \
    --error="$SLURM_LOGS/eval_perf_rc_bce_small4_chained_%j.out" \
    --wrap="cd $REPO_ROOT && \
...
echo "Submitted perf_rc_chain_bce_s4. Logs: $SLURM_LOGS/eval_perf_rc_bce_small4_chained_<jobid>.out"
