#!/usr/bin/env bash
# Chained subtask evaluation for no-qwen3 perf router on R+C questions (GPU).
# Runs eval_perf_router.py with --qid-filter, producing full-task + overall + chained.
# Overwrites rc_questions/no_qwen3/subtask_summary.json with chained fields included.
#
# Prerequisites:
...
RESULTS_DIR="$REPO_ROOT/perf_router/results/rc_questions/no_qwen3"
...
CKPT="/n/fs/scratch/dl3533/models/perf_router/perf_router_no_qwen3_best.pt"

sbatch \
    --job-name="perf_rc_chain_nq" \
    --account=allcs \
    --partition=pvl \
    --gres=gpu:a6000:1 \
    --time=0:20:00 \
    --mem=16G \
    --cpus-per-task=2 \
    --output="$SLURM_LOGS/eval_perf_rc_no_qwen3_chained_%j.out" \
    --error="$SLURM_LOGS/eval_perf_rc_no_qwen3_chained_%j.out" \
    --wrap="cd $REPO_ROOT && \
...
echo "Submitted perf_rc_chain_nq. Logs: $SLURM_LOGS/eval_perf_rc_no_qwen3_chained_<jobid>.out"
